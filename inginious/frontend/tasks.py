...
class WebAppTask(Task):
    ...
    def get_context(self, language):
        ...
        return ParsableText(vals[0], "rst", language=language, translation=self._translations.get(language, gettext.NullTranslations())) if len(vals) \
            else ParsableText(context, "rst", language=language, translation=self._translations.get(language, gettext.NullTranslations()))
